...
Recounted **2026-08-13**: **223** `rtx.dusklight.*` names declared across the
`RTX_OPTION*` surfaces against **131** present in the file, so it is **missing
92**, and it still lists exactly **one** that no longer exists,
`emissive.intensity`, renamed to `brightness`. The declarations in
`rtx_dusklight_*.h` are the authority; a name absent from `RtxOptions.md` is not
evidence it does not exist. Regenerate on the next Windows run.
