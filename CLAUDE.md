...
- the 29 `rtx.dusklight.game.effectLight*` options and the 14
  ...
  `effectLightIntensity` / `ReachScale` / `RadiusScale`, one per value the
...
| `src/dxvk/rtx_render/rtx_dusklight_atmosphere.{h,cpp}` | one medium driving fog, sky and sky-light; Hillaire physical sky. **Reworked 2026-08-13, fork-only, no protocol bump:** the game's ramp owns the fog's *appearance* at every distance (applied as an exact residual against the medium, `fogRampMode` 1, still the default) and the medium only owns the *light*, so σ can be capped until it never out-fogs the original's clear zone. The fog's colour is measured off the generated dome by a new `dusklight_sky_stats` reduction pass with a non-stalling readback — necessary because **the RTXDI light list has no dome light**, so the froxel grid is never lit by the sky. **Four further fork-only passes on 2026-08-17/18, again with no protocol bump, all UNTESTED IN GAME:** (1) the anchor match is to the game's *actual* opacity there rather than to half — identical where the `zHalfMin` clamp is idle, **4.46× denser where it binds**, which is every scripted fog bank; (2) both halves of the fog now blend towards **one** colour, because the medium's isotropic in-scatter used the dome's sphere mean while the far ramp sampled the same dome *directionally* — the §5.2 identity had two `A`s in it and nobody noticed for four days (`fogColorDirectional`, default false, restores the tint); (3) both *paths* now share one derivation — `applyFogOverride` writes the resolved radiance into `FogState::color` and `rtx_composite.cpp` **skips `rtx.fogColorScale` for it** (gated on `fogColorOverridden()`, not `active()`: the override declines to write when a translucent material already replaced the fog, and the raw colour left there is exactly what the scale is for), so that option no longer affects Dusklight's fog in the ordinary case and `fogRadianceScale` is the level knob for both; (4) **`fogRampMode` 2**, which gives the medium `σ(d) = 1/(end − d)` — the game's fog *is* a heterogeneous participating medium — implemented by shadowing the RTXDI submodule's `sampleDensityField`, see the shader note below. Derivations and what is still owed: `documentation/DusklightAtmosphere.md` §5.1–§5.4 |
| `src/dxvk/shaders/rtx/algorithm/volume_lighting.slangh` | **the only file in this fork that is neither new nor a copy of an upstream one.** It **shadows** the RTXDI submodule's file of the same relative path (`submodules/rtxdi/rtxdi-sdk/include/volumetrics/…`), which resolves because `meson.build` passes `src/dxvk/shaders` as an `-I` root before `volumetrics_include_path_string`. **The volumetric shaders are not missing from this repository — they are in the RTXDI submodule**, and any document saying otherwise is wrong. It renames one function out of the way with a macro, includes the submodule's copy verbatim, and defines a replacement over it, so an RTXDI bump is picked up automatically and an upstream signature change is a compile error rather than silent drift. Deleting it loses `fogRampMode` 2 and nothing else. `DusklightAtmosphere.md` §11 has what a rebase must check |
...
Recounted **2026-08-18**: **231** `rtx.dusklight.*` names declared across the
`RTX_OPTION*` surfaces against **124** present in the file, so it is **missing
107**. It also lists **eight** that no longer exist — `emissive.intensity`
(renamed to `brightness`) and the seven-row `localLights` family
(`game.localLights` / `localLightIntensity` / `localLightRadius` and the four
`env.localLights*` readouts), removed at protocol 17 on 2026-08-16. The
declarations in `rtx_dusklight_*.h` are the authority; a name absent from
`RtxOptions.md` is not evidence it does not exist. Regenerate on the next Windows
run.

**Do not restate these numbers from memory — `scripts/check_dusklight_invariants.py`
prints the live count on every run, and that is the figure to quote.** The
...
>
> **And it happened again, quietly, between 2026-08-13 and 2026-08-18** — with no
> merge involved this time, just ordinary work. The paragraph above said "missing
> 92" and "still lists **exactly one** that no longer exists". Nothing touched the
> line; the protocol-17 removals and the options added since made both figures
> false, and the "exactly one" had by then grown to eight. **Run the script; do
> not read the number off this page.**
