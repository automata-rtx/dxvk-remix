...
| `src/dxvk/rtx_render/rtx_dusklight_skeleton.{h,cpp}` | `rtx.dusklight.skeleton.*` — character merge and real armatures. Holds the two exports the other two repos resolve by name (`dusklight_DeclareSkeleton`, `dusklight_SetDrawSkeleton`), the per-model joint tree, `groupMeshHash(modelKey)`, and the per-frame claim that keeps one instance's siblings from each drawing the replacement. **The group hash must depend on the model key alone** — a hash mixing in the member draw set is computable by the capture and by nothing else, so a replacement authored against it binds to nothing, silently. A three-repo feature: `dusklight-ao/src/dusk/remix_skeleton.cpp` publishes the joint tree, `aurora-ao/lib/dx9/` composes its palette compaction. **Untested in game**; the first build of it crashed on launch |
