...
merged and after the haze-blend and protocol-12 work: **170** declared against
**132** rows in the file, of which **131** are still declared — so it is
**missing 39** and lists exactly **one** that no longer exists,
`emissive.intensity`, renamed to `brightness`. The 39 are the 20
`game.effectLight*`, the 11 `env.effLights*` readouts, `catrepCommit`,
`game.lanternInfiniteOil`, the three `atmosphere.kasumi*` haze-blend options and
the three `env.*Alpha` readouts.
