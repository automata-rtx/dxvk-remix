...
**Protocol is at 12.** Build both sides from the same commit point, and bump
...
Recounted **2026-08-11**, on a rebase carrying the water/matrep work, the
haze-blend work and protocol 12: **191** declared against **132** rows in the
file, of which **131** are still declared — so it is **missing 60** and lists
exactly **one** that no longer exists, `emissive.intensity`, renamed to
`brightness`.

**Do not maintain that number by hand.** It has been wrong three times: the row
count was stated as 133 when it was 132, and the declared count has since moved
twice as branches merged. The command below prints all of it.

Reproduce the count rather than trusting this paragraph — it has been wrong
before, and the row count above is **132**, not the **133** an earlier revision
of this section claimed:

```
python3 scripts/check_dusklight_invariants.py     # prints the missing count
``` The declarations in `rtx_dusklight_*.h` are the authority; a
...
- **A readout whose valid range includes its default needs a sentinel.** Only
  the game writes `env.*`, so against a game older than the protocol that added
  one, the option keeps its default — indistinguishable from the game pushing
  that value. The protocol-12 palette alphas default to **-1** because `0` is a
  legal authored value, and reading an old game's silence as `0` would have
  dropped the near haze band and looked like an artistic choice.
...
Once this project establishes that a sentence is *false*, its words must not
reappear anywhere in the tree. That exists because of a specific failure: the
kasumi haze bands were corrected from "on the sun's side"/"away from the sun" to
front/back on 2026-08-10, the option descriptions and `.md` files were fixed, and
**the shader that reads both fields kept the wrong comment and the blend built on
it for another day.** Nothing was looking.

It cannot tell whether prose is true. It only enforces "we already decided this
sentence is false", which is the part that recurs — a retired claim gets re-typed
from memory by the next person who half-remembers it. Documents whose subject *is*
the correction may quote the old wording; say so within a couple of lines
(in prose, anywhere in the same paragraph) or put it in a fenced block.

**Adding an entry is deliberate and takes the correction with it**, so the next
reader learns what is true rather than only what is banned. **When you correct a
claim, add its old words here in the same commit** — that is the whole mechanism.
...
- whether a document's *prose* still describes reality, as opposed to its
  numbers agreeing with the code — the retired-claims list catches only sentences
  already known to be false, never a new one
