...
| `rtx_instance_manager.cpp` | **a second Dusklight site in this file**, unrelated to the emissive patch a few rows up: `surface.associatedGeometryHash` takes the group hash for a bound draw, so a character paints one colour in the Geometry Hash debug view. `rtx_materials.h` documents that field as debug-view-only and nothing else keys on it, so this changes no rendering, no BLAS caching and no tagging. `RtSurface` does not grow, so the size tripwire is not involved | `rtx.dusklight.skeleton.enable` |
