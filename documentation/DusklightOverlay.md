...
| Mods tab | landed 2026-08-15, protocol 16 — **not built, not tested in game.** Re-enables mod discovery under the D3D9 backend and adds a **Mods** tab, because the game's own mod window is never drawn in fixed-function mode and so a mod was unreachable. The game publishes `modsRunning`, `modCount` and `modList` (`id|name|native|active|failed`, records `;` separated); the tab writes back `modsEnabled`, a `|` list of ids the game diffs into `ModLoader::request_enable`/`request_disable`. **`modsEnabled` is `NoSave` and that is the feature, not housekeeping** — the requirement is that mods are always off at startup, and a persisted list would silently re-enable one next launch, including one that crashed the process on load. The game ignores its own `config.json` enable state for the same reason. `-` is the "none" sentinel because an empty string never crosses the wire. **The native warning is load-bearing**: mod discovery was switched off under this backend precisely because the D3D9 path never initializes WebGPU, so a native mod that touches the renderer takes the process down as it loads — which is the instant its box is ticked. Nothing in the runtime loads or executes mod code; it shows an inventory and returns a list of ids |
